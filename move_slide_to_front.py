...
def move_slide(prs, old_index, new_index):
    xml_slides = prs.slides._sldIdLst  # the XML element containing the slide references
    ...
    for file in pptx_files:
        if should_update_file(file):
            try:
                ...
                if details_idx != -1 and details_idx > 1:
                    # Move to index 1 (right after the title slide)
                    move_slide(prs, details_idx, 1)
                    prs.save(file)
                    print(f"Moved details slide in {file}")
                else:
